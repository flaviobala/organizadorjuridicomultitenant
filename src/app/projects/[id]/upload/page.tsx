...
export default function UploadDocumentsPage({
  ...
  // ✅ NOVO: Estado para modal de qualidade do OCR
  const [ocrQualityModal, setOcrQualityModal] = useState<{
    filename: string
    confidence: number
    charactersExtracted: number
    pagesProcessed: number
    qualityLevel: 'excellent' | 'good' | 'poor' | 'failed'
    ocrText?: string
  } | null>(null)
    ...
    try {
      ...
      for (let i = 0; i < previewFiles.length; i++) {
        ...
        if (result.success) {
          ...
          // ✅ Capturar dados de qualidade do OCR do último documento processado
          if (i === previewFiles.length - 1 && result.ocrQuality) {
            // Mostrar modal apenas para o último documento
            setOcrQualityModal({
              filename: file.name,
              confidence: result.ocrQuality.confidence || 0,
              charactersExtracted: result.ocrQuality.charactersExtracted || 0,
              pagesProcessed: result.ocrQuality.pagesProcessed || 1,
              qualityLevel: result.ocrQuality.qualityLevel || 'failed',
              ocrText: result.ocrQuality.ocrText
            })
          }
        } else {
          setUploadProgress(prev => ({ ...prev, [fileKey]: 0 }))
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full">
            ...
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center space-x-3">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${
                  ocrQualityModal.qualityLevel === 'excellent' ? 'bg-green-100' :
                  ocrQualityModal.qualityLevel === 'good' ? 'bg-yellow-100' :
                  ocrQualityModal.qualityLevel === 'poor' ? 'bg-orange-100' : 'bg-red-100'
                }`}>
                  {ocrQualityModal.qualityLevel === 'excellent' && <CheckCircle className="w-6 h-6 text-green-600" />}
                  {ocrQualityModal.qualityLevel === 'good' && <AlertTriangle className="w-6 h-6 text-yellow-600" />}
                  {ocrQualityModal.qualityLevel === 'poor' && <AlertTriangle className="w-6 h-6 text-orange-600" />}
                  {ocrQualityModal.qualityLevel === 'failed' && <X className="w-6 h-6 text-red-600" />}
                </div>
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-gray-900">Qualidade da Extração</h3>
                  <p className="text-sm text-gray-500">{ocrQualityModal.filename}</p>
                </div>
                <button
                  onClick={() => setOcrQualityModal(null)}
                  className="p-2 text-gray-400 hover:text-gray-600 transition-colors cursor-pointer flex-shrink-0"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="text-2xl font-bold text-gray-900">{ocrQualityModal.confidence}%</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">Caracteres</p>
                    <p className="text-2xl font-bold text-gray-900">{ocrQualityModal.charactersExtracted.toLocaleString()}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">Páginas</p>
                    <p className="text-2xl font-bold text-gray-900">{ocrQualityModal.pagesProcessed}</p>
                  </div>
                </div>

                {/* Status Message */}
                <div className={`rounded-lg p-4 ${
                  ocrQualityModal.qualityLevel === 'excellent' ? 'bg-green-50 border border-green-200' :
                  ocrQualityModal.qualityLevel === 'good' ? 'bg-yellow-50 border border-yellow-200' :
                  ocrQualityModal.qualityLevel === 'poor' ? 'bg-orange-50 border border-orange-200' : 'bg-red-50 border border-red-200'
                }`}>
                  {ocrQualityModal.qualityLevel === 'excellent' && (
                    <>
                      <p className="text-sm font-medium text-green-900">✅ Excelente qualidade</p>
                      <p className="text-sm text-green-700 mt-1">
                        O texto foi extraído com alta precisão. Você pode usar os dados com confiança.
                      </p>
                    </>
                  )}
                  {ocrQualityModal.qualityLevel === 'good' && (
                    <>
                      <p className="text-sm font-medium text-yellow-900">⚠️ Qualidade moderada</p>
                      <p className="text-sm text-yellow-700 mt-1">
                        O texto foi extraído, mas pode conter alguns erros. Recomendamos revisar os dados extraídos.
                      </p>
                    </>
                  )}
                  {ocrQualityModal.qualityLevel === 'poor' && (
                    <>
                      <p className="text-sm font-medium text-orange-900">⚠️ Má qualidade</p>
                      <p className="text-sm text-orange-700 mt-1">
                        Documento com baixa qualidade. Muitos erros podem ter ocorrido na extração.
                      </p>
                      <p className="text-sm text-orange-700 font-medium mt-2">
                        💡 Sugestão: Faça novo scan em melhor qualidade ou tire nova foto com melhor iluminação.
                      </p>
                    </>
                  )}
                  {ocrQualityModal.qualityLevel === 'failed' && (
                    <>
                      <p className="text-sm font-medium text-red-900">❌ Falha na extração</p>
                      <p className="text-sm text-red-700 mt-1">
                        Não foi possível extrair texto significativo do documento.
                      </p>
                      <p className="text-sm text-red-700 font-medium mt-2">
                        💡 Sugestão: Verifique se o documento está legível e tente fazer upload novamente com melhor qualidade.
                      </p>
                    </>
                  )}
                </div>
            ...
            <div className="px-6 py-4 bg-gray-50 flex justify-end gap-2 rounded-b-lg">
              {ocrQualityModal.ocrText && (
                <button
                  onClick={() => {
                    setOcrQualityModal(null)
                    setSelectedDocumentOcr({
                      filename: ocrQualityModal.filename,
                      text: ocrQualityModal.ocrText || ''
                    })
                    setOcrModalOpen(true)
                  }}
                  className="px-4 py-2 text-sm bg-purple-600 text-white rounded-md hover:bg-purple-700 transition-colors cursor-pointer"
                >
                  Ver Texto Extraído
                </button>
              )}
